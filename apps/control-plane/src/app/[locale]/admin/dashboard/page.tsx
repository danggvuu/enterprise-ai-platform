...
export default function DashboardPage() {
  ...
  if (isError || !data) {
    return (
      <div className="flex flex-col items-center justify-center p-12 border border-dashed border-zinc-800 rounded-xl">
        ...
        <p className="text-zinc-500 text-sm mt-1">Please ensure the gateway server is running on port 3000.</p>
        <button
          onClick={() => refetch()}
  ...
  const statCards = [
    { title: 'Total Requests', value: data.totalRequests, change: '+12%', icon: Activity, color: 'text-blue-500' },
    { title: 'Average Latency', value: `${Math.round(data.averageLatency)} ms`, change: '-4%', icon: Clock, color: 'text-amber-500' },
    { title: 'Cache Hit Ratio', value: `${(data.cacheHitRatio * 100).toFixed(1)}%`, change: '+1.5%', icon: Database, color: 'text-emerald-500' },
    { title: 'Today Cost (USD)', value: `$${data.todayCostUsd.toFixed(4)}`, change: '-8%', icon: Coins, color: 'text-emerald-400' },
    { title: 'PII Shield Blocks', value: data.detectedPii, change: '0', icon: Shield, color: 'text-blue-400' },
    { title: 'Security Violations', value: data.blockedPrompts, change: '0', icon: AlertCircle, color: 'text-red-500' },
    { title: 'Circuit Breaker Events', value: data.circuitBreakerEvents, change: '0', icon: RefreshCw, color: 'text-purple-400' },
    { title: 'Active Users', value: data.activeUsers, change: '+2', icon: Shield, color: 'text-zinc-400' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-zinc-100">Operational Dashboard</h1>
          <p className="text-zinc-500 text-xs mt-1">Live telemetry sync from the API gateway core.</p>
        </div>
        <button
          onClick={() => refetch()}
      ...
      <div className="grid grid-cols-1 gap-8">
        <div className="bg-zinc-900 border border-zinc-850 p-6 rounded-xl space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-zinc-400">Gateway Cost Pipeline (USD)</span>
            <span className="text-[10px] text-zinc-500">Live Area Chart</span>
          </div>
      ...
      <div className="bg-zinc-900 border border-zinc-850 p-6 rounded-xl space-y-4">
        <h3 className="text-sm font-bold uppercase tracking-wider text-zinc-400">Recent Gateway Operations</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-left border-collapse">
            <thead>
              <tr className="border-b border-zinc-800 text-zinc-500">
                <th className="py-2.5 px-4 font-semibold">User</th>
                <th className="py-2.5 px-4 font-semibold">Prompt Preview</th>
            ...
            <tbody>
              {data.recentRequests.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-zinc-500">No recent request logs recorded.</td>
                </tr>
              ) : (
