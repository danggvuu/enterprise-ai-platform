...
export default function SecurityPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Title */}
      <div>
        <h1 className="text-2xl font-extrabold text-zinc-100">Security & Guardrails</h1>
        <p className="text-zinc-500 text-xs mt-1">Audit log of Prompt Injection attempts, Vietnamese PII detections, and policy shields.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-zinc-900 border border-zinc-850 p-5 rounded-xl space-y-2 flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-extrabold text-blue-400">{events.filter(e => e.type === 'PII Detection').length}</div>
          </div>
        ...
        <div className="bg-zinc-900 border border-zinc-850 p-5 rounded-xl space-y-2 flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-extrabold text-red-500">{events.filter(e => e.type === 'Prompt Injection').length}</div>
          </div>
              ...
              {events.length === 0 ? (
                ...
                events.map((ev, idx) => (
                  <tr key={idx} className="border-b border-zinc-800/40 hover:bg-zinc-850/20 text-zinc-300">
                    ...
                    <td className="py-2.5 px-4 font-medium">{ev.user}</td>
                    <td className="py-2.5 px-4">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        ev.type === 'Prompt Injection' ? 'bg-red-500/10 text-red-400' : 'bg-blue-500/10 text-blue-400'
                      }`}>
                        {ev.type}
                      </span>
                    </td>
